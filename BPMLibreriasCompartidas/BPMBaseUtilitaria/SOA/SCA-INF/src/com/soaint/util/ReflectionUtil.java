...
public class ReflectionUtil {
    public static final ADFLogger LOGGER =
        ADFLogger.createADFLogger(ReflectionUtil.class);
    ...
    public static void cloneObject(Object fuente, Object destino) {
        if (fuente == null || destino == null ||
            !fuente.getClass().equals(destino.getClass())) {
            return;
        }
        ...
        for (int i = 0; i < attr.length; i++) {
            ...
            if (!java.lang.reflect.Modifier.isStatic(temp.getModifiers())) {
                try {
                    Field tempDes = destino.getClass().getDeclaredField(name);
                    tempDes.setAccessible(true);
                    tempDes.set(destino, temp.get(fuente));
                } catch (IllegalAccessException e) {
                    LOGGER.warning("Error en el cloneObject \" " + name +
    ...
    public static void cleanObject(Object o) {
    ...
    public static Map<?, ?> convertListToMap(List<?> lista, String key) {
        ...
        try {
            ...
            LOGGER.warning("Error en el convertListToMap \" " + key +
                           "\", originado por :  " + e.getMessage());
        } catch (Exception e) {
            LOGGER.warning("Error en el convertListToMap \" " + key +
                           "\", originado por :  " + e.getMessage());
